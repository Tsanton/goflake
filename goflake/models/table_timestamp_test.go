...
func Test_create_table_timestamp(t *testing.T) {
	/* Arrange */
	cli := i.Goflake()
	defer cli.Close()
	...
	col1 := a.Timestamp{
		Scale:         7,
		TimestampType: enums.SnowflakeTimestampLtz,
		DefaultValue:  nil,
		Nullable:      false,
		Unique:        false,
	...
	assert.Equal(t, string(enums.SnowflakeTimestampLtz), dbTable.Columns[0].ColumnType.Type)
